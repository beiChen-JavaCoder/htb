package com.xqf.srb.core.hfb;

import com.alibaba.fastjson.JSONObject;
import com.xqf.common.util.HttpUtils;
import com.xqf.common.util.MD5;
...
public class RequestHelper {
    ...
    public static JSONObject sendRequest(Map<String, Object> paramMap, String url){
        ...
        try {
            ...
            log.info(String.format("--> 发送请求到航汇通：post data %1s", postdata));
            byte[] reqData = postdata.toString().getBytes("utf-8");
            byte[] respdata = HttpUtils.doPost(url,reqData);
            result = new String(respdata);
            log.info(String.format("--> 航汇通应答结果：result data %1s", result));
        } catch (Exception ex) {
            ex.printStackTrace();
        }
